...
def make_right_call(device : dict, method):
    if 'fridge' in device.keys():
        ...
        turnedOn=device['turnedOn'],usedEnergyOverall=device['usedEnergyOverall'],temperature=house_pb2.TemperatureDetails(
            ...
            highBound=device['temperature']['highBound']),fridge=house_pb2.FridgeDetails(
                fridgeFilled=device['fridge']['fridgeFilled'],timer=house_pb2.Time(hour=device['fridge']['timer']['hour'],
                minutes=device['fridge']['timer']['minutes']))))
    elif 'furnace' in device.keys():
        return method(house_pb2.Device(ID=device['ID'],type=device['type'],
        turnedOn=device['turnedOn'],usedEnergyOverall=device['usedEnergyOverall'],temperature=house_pb2.TemperatureDetails(
            currentTemperature=device['temperature']['currentTemperature'],lowBound=device['temperature']['lowBound'],
            highBound=device['temperature']['highBound']),furnace=house_pb2.FurnaceDetails(
                emittedCO=device['furnace']['emittedCO'],mode=device['furnace']['mode'])))
    elif 'regulator' in device.keys():
        if 'heater' in device['regulator'].keys():
            return method(house_pb2.Device(ID=device['ID'],type=device['type'],
            turnedOn=device['turnedOn'],usedEnergyOverall=device['usedEnergyOverall'],temperature=house_pb2.TemperatureDetails(
                currentTemperature=device['temperature']['currentTemperature'],lowBound=device['temperature']['lowBound'],
                highBound=device['temperature']['highBound']),regulator=house_pb2.RegulatorDetails(
                    energy=device['regulator']['energy'],heater=house_pb2.Heater(degree=device['regulator']['heater']['degree'],
            ...
            turnedOn=device['turnedOn'],usedEnergyOverall=device['usedEnergyOverall'],temperature=house_pb2.TemperatureDetails(
                currentTemperature=device['temperature']['currentTemperature'],lowBound=device['temperature']['lowBound'],
                highBound=device['temperature']['highBound']),regulator=house_pb2.RegulatorDetails(
                    energy=device['regulator']['energy'],
